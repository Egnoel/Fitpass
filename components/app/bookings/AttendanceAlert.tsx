...
import type { USER_BOOKINGS_QUERY_RESULT } from "@/sanity.types";

type Booking = USER_BOOKINGS_QUERY_RESULT[number];
...
export function AttendanceAlert({ bookings }: AttendanceAlertProps) {
    ...
    const needsAttendance = bookings.filter((booking) => {
        const status = booking.status ? (String(booking.status) as "attended" | "cancelled" | "confirmed" | "noShow") : null;
