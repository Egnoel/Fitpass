...
import { FILTERED_SESSIONS_QUERY_RESULT } from "@/sanity.types";
...
type SessionWithDistance = Session & { distance: number };
...
export default async function ClassesPage({ searchParams }: PageProps) {
    ...
    const allSessions = sessionsResult.data as unknown as Session[];
    ...
    const sessionsForFilter: Session[] = allSessions
    ...
    const groupedArray: [string, SessionWithDistance[]][] = Array.from(groupedByDay.entries());

    // Extract venues for map display
    const venuesForMap: NonNullable<(typeof sessionsWithDistance)[number]["venue"]>[] =
        sessionsWithDistance.flatMap((s) => (s.venue ? [s.venue as NonNullable<(typeof sessionsWithDistance)[number]["venue"]>] : []));
                ...
                <div className="flex gap-6">
                    ...
                    <aside className="hidden w-[400px] shrink-0 xl:block">
                        <Card className="sticky top-20 h-[calc(100vh-8rem)] overflow-hidden p-0">
                            <ClassesMapSidebar
                                venues={venuesForMap as NonNullable<FILTERED_SESSIONS_QUERY_RESULT[number]["venue"]>[]}
                                userLocation={{ lat: location.lat ?? 0, lng: location.lng ?? 0 }}
                            />
                        </Card>
                    </aside>
                </div>
